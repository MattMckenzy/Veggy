...
public partial class SettingsList
{
    ...
    private async Task LoadSetting((string _, string __, string Key) item)
    {
        ...
        async void loadAction()
        {
            ...
            await SetCurrentSetting((Setting)veggyContext.Entry(setting).CurrentValues.Clone().ToObject());
    ...
    private async Task SaveSetting()
    {
        ...
        await veggyContext.Settings.SetSettingAsync(CurrentSetting.Name, CurrentSetting.Value);
    ...
    private async Task UpdatePageState()
    {
        ...
        CurrentSettingIsDirty = !string.IsNullOrWhiteSpace(CurrentSetting?.Name) && !CompareSettings(CurrentSetting, veggyContext.Settings.ToArray().FirstOrDefault(setting => setting.Name.Equals(CurrentSetting.Name)));

        Settings = [.. veggyContext.Settings];
