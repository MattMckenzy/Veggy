...
public class StorageService(ProtectedLocalStorage protectedLocalStorage, GotifyService gotifyService, ILogger<StorageService> logger)
{
    ...
    public async Task<string?> Get(string token)
    {
        ...
        {
            await GotifyService.PushMessage("Veggy Debug", $"Client token out of date, couldn't retrieve data from storage.", LogLevel.Debug);
            Logger.LogInformation("Debug ({dateTime}) - Client token out of date, couldn't retrieve data from storage.", DateTime.Now);        
        }
        catch (Exception exception)
        {
            await GotifyService.PushMessage("Veggy Error", $"Error during retrieval of data from storage: {exception.Message + Environment.NewLine + exception.StackTrace}", LogLevel.Error);
    ...
    public async Task Set(string token, object value)
    {
        ...
        {
            await GotifyService.PushMessage("Veggy Error", $"Error during storage of data: {exception.Message + Environment.NewLine + exception.StackTrace}", LogLevel.Error);
